...
- ~~Document data residency requirements per jurisdiction~~ — region stamp [ADR-025](../adr/ADR-025-residency-erasure-ledger.md); transfer intent [ADR-026](../adr/ADR-026-sealed-cold-erasure-transfers.md); execute pipeline [ADR-027](../adr/ADR-027-automated-transfer-pipeline.md)
...
- ~~Hardware WORM / crypto-shred / IdP portal facades~~ — **ADR-034.** ~~SoA draft / OIDC RP~~ — **ADR-036.** ~~Auditor-signed SoA / SAML SP~~ — **ADR-041.** ~~Auditor evidence packs / SAML federation~~ — **ADR-044.** ~~Auditor-issued certs / XMLDSig~~ — **ADR-045.** ~~Mesh / LGU foundation arc complete~~ — **ADR-046.** ~~Production soft on-ramps~~ — **ADR-047.** Accredited ISO / HSM custody remain contract-gated ([mesh-lgu-foundation-arc.md](mesh-lgu-foundation-arc.md))
