...
| C-19 | Self-hosted models via vLLM/Ollama | Adapter (behind LLMPort) | §6 Phase 3 | Foundation — ADR-016 (`deploy/ollama/`) + ADR-048 (`deploy/vllm/`) |
...
**Phase 3 — trigger: JOTHAM revenue funds a GPU.** Self-hosted models via vLLM/Ollama behind `LLMPort` — adapters + Ollama ops (ADR-016) + soft vLLM kit (ADR-048) landed; NVIDIA toolkit, model weights, auth proxy remain operator-owned until the trigger.
