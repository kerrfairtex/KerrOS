...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _KERROS_env="${KERROS_HOME:-$HOME/.KerrOS}/.env"; [ -f "$_KERROS_env" ] && grep -q "^GITHUB_TOKEN=" "$_KERROS_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_KERROS_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${KERROS_HOME:-$HOME/.KerrOS}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
```bash
source "${KERROS_HOME:-$HOME/.KerrOS}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by KerrOS. Code looks clean — good test coverage, no security concerns."
