...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the KerrOS terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For KerrOS itself, `model.provider: openai-codex` uses KerrOS-managed Codex
OAuth from `~/.kerros/auth.json` after `KerrOS auth add openai-codex`. For the
...
## KerrOS Gateway Caveat

When invoking the Codex CLI from a KerrOS gateway/service context (for example,
