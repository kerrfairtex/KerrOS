...
Everything operator-facing is a `KerrOS teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${KERROS_HOME:-~/.kerros}/.env`:
...
```bash
KerrOS teams-pipeline validate              # config snapshot — run first after any change
KerrOS teams-pipeline token-health          # Graph token status
KerrOS teams-pipeline token-health --force-refresh   # force a fresh token acquisition
KerrOS teams-pipeline list                  # recent meeting jobs
KerrOS teams-pipeline list --status failed  # only failed jobs
KerrOS teams-pipeline show <job-id>         # full detail of one job
KerrOS teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
KerrOS teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
KerrOS teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
KerrOS teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
KerrOS teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

KerrOS teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
KerrOS teams-pipeline delete-subscription <sub-id>
KerrOS teams-pipeline maintain-subscriptions            # renew near-expiry ones
KerrOS teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `KerrOS teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `KerrOS cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
